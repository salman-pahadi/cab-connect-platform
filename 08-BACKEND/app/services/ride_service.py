...
class RideService:
    ...
    def start_ride(self, db: Session, ride_id: int, otp_code: str) -> Ride:
        ...
        if ride_obj.status != RideStatus.ACCEPTED:
    ...
    def complete_ride(
        ...
        if ride_obj.status != RideStatus.IN_PROGRESS:
        ...
        payment = Payment(
            transaction_id=f"PAY{ride_obj.ride_number}",
            ride_id=ride_obj.id,
            passenger_id=ride_obj.passenger_id,
            driver_id=ride_obj.driver_id,
            amount=completion_data.actual_fare,
            payment_method=ride_obj.payment_method.value,
            base_fare=50.0,
            ...
            status=TransactionStatus.SUCCESS if ride_obj.payment_method == PaymentMethod.CASH else TransactionStatus.PENDING,
        )
    ...
    def get_ride(self, db: Session, ride_id: int) -> Ride | None:
        """Get ride by ID."""
        result: Ride | None = db.query(Ride).filter(Ride.id == ride_id).first()
        return result
